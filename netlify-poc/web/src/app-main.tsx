...
import { apiGet, type Whoami } from "./identity";
  ...
  return (
    <section style={{ border: "1px solid #ddd", borderRadius: 6, padding: "0.75rem 1rem", marginBottom: "1rem" }}>
      ...
      {state.err ? <pre style={{ color: "#b42318", whiteSpace: "pre-wrap" }}>{state.err}</pre>
        : state.ok ? <pre style={{ margin: 0, whiteSpace: "pre-wrap" }}>{JSON.stringify(state.ok, null, 2)}</pre>
        : <em>loading…</em>}
    </section>
  );
}
...
function App() {
  return (
    <main style={{ fontFamily: "system-ui", maxWidth: 720, margin: "3rem auto", lineHeight: 1.5 }}>
      <h1>citizen-poc-netlify — app</h1>
      <p>Behind Netlify's SSO team login. Compare each panel with <code>hello-citizen</code> on the custom playground.</p>
      <Panel title="GET /api/me — what the platform told the app about you" load={() => apiGet<Whoami>("/api/me")} />
      ...
      <Panel title="GET /api/fabric — Fabric SQL endpoint via a service principal secret held in env vars" load={() => apiGet<Fabric>("/api/fabric")} />
    </main>
  );
}
